...
export async function main(
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("join-as-new-peer", async ({ roomId, peerId, peer }) => {
      socket.join(roomId);
      if (!(roomId in rooms)) {
        rooms[roomId] = createRoom(socket, roomId);
      }
    ...
    socket.on("add-speaker", async ({ roomId, peerId }) => {
      ...
      console.log(rooms[roomId]);
      io.to(socket.id).emit("you-are-now-a-speaker", {
        roomId,
        sendTransportOptions: transportToOptions(sendTransport),
      });

      io.to(roomId).emit("speaker-added", {roomId, peerId})
    ...
    socket.on("user-asked-to-speak", ({ peerId, roomId }) => {
      io.to(roomId).emit("hand-raised", { peerId, roomId });
    });

    socket.on("new-mod", ({ peerId, roomId }) => {
      console.log("about to create new mod");
      io.to(roomId).emit("new-mod", { peerId, roomId });
    });
  });
}
